...
# 		if request.user.is_teacher() and text_language == 1:
# 			return render(request, "work_area_legacy.html", context={
# 				'founded': True,
# 				'ann_right': ann_right,
...
# 			})
# 	else:
# 		return render(request, 'work_area_legacy.html', context={'founded': False})
...
def show_text(request, text_id):
	text = TblText.objects.filter(id_text=text_id).values('language_id', 'text_type_id', 'user_id')
	if not text.exists():
		return render(request, 'text_annotation.html', context={ 'exists': False })
	text = text.first()

	if not (request.user.is_authenticated and check_permissions_show_text(request.user.id_user, text_id)):
		return render(request, 'access_denied.html', status=403)
	...
	return render(request, 'text_annotation.html', context=context)
