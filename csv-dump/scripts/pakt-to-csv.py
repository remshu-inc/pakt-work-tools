...
SQL_QUERY_PATH = os.path.dirname(os.path.abspath(__file__)) + '/source/'
SAVE_PATH = os.path.dirname(os.path.abspath(__file__)) + '/../data/'
# * Connection
login = input('User name: ')
password = getpass.getpass('Password: ')
database_name = input('Database name: ')
engine = sqlalchemy.create_engine(f"mysql+pymysql://{login}:{password}@localhost:3306/{database_name}?charset=utf8mb4",
                                  echo=False)
